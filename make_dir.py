    ...
    for i in range(len(li)):
        ...
        try:
            if(z.find('### Example Input')==-1):
                print(1/0)
            inp=z[z.find("### Example Input")+21:z.find("### Example Output")-6]
        except:
            inp=""
        try:
...
def test_ques1(contest_code,s):
    ...
    i=0;
    while i<1:
        obj=s.get(f"https://www.codechef.com/status/{contest_code}")
...
def test_ques(li,contest_code,s):
    ...
    while(i<len(li)):
        ...
        obj=s.get(f"https://www.codechef.com/{contest_code}/status/{li[i]}?sort_by=All&language=44&status=15&Submit=GO")
        soup=bs4.BeautifulSoup(obj.text,'html.parser')
        ans=soup.text
        #print(type(ans))
        ans=(ans[ans.find("IDDate/TimeUserResultTimeMemLangSolution")+40:])
